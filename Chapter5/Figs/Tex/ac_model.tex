...
\begin{tikzpicture}[>=stealth,every node/.style={shape=rectangle,draw},]
	% create the nodes
	\node (users){Users};
	\node (roles)[right =of users, text width=3.5cm]{Participant Roles\\ eg. Teachers, Learners};
	\node (functions)[right =of roles, text width=2.1cm]{Transactions\\(optional)};
	\node (conditions)[right =of functions, text width=2.5cm]{Attribute-based\\Conditions\\(optional)};	
	\node (permissions)[right =of conditions, text width=1.8cm]{Permissions};
	\node (operation)[below left = 1cm and -0.5cm of permissions, text width=2cm]{Operation(s)};
	\node (object)[below right = 1cm and -1cm of permissions, text width=3.75cm]{Object\\eg. Assets, Participants};
	...
	% connect the nodes
	\draw (users) -- (roles);
	\draw (roles) -- (functions);
	\draw (functions) -- (conditions);
	\draw (conditions) -- (permissions);
	\draw (permissions) -- (operation);
	\draw (permissions) -- (object);
