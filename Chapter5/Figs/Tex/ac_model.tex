...
\begin{tikzpicture}[>=stealth,every node/.style={shape=rectangle,draw},]
	...
	\node (functions)[right =of roles, text width=2.25cm]{Transactions\\(optional)};
	\node (conditions)[right =of functions, text width=3.5cm]{Attribute-based\\Conditions (optional)};	
	\node (permissions)[right =of conditions, text width=2.25cm]{Permissions};
	\node (operation)[below left = 1cm and -1cm of permissions, text width=2.5cm]{Operation(s)};
	\node (object)[below right = 1cm and -1cm of permissions, text width=4cm]{Object\\eg. Assets, Participants};
