...
\begin{tikzpicture}[>=stealth,every node/.style={shape=rectangle,draw,rounded corners},]
    ...
    \node (auto1) [right =of c1, text width=3cm, align=center]{Send the test file and submission file to the automatic testing service};
    ...
    % connect the nodes
    \draw[->] (start) to (c1);
    \draw[->] (c1) -- node[anchor=south] {if Automatic Assessment} (auto1);
    % \draw[->] (c1) -- node[anchor=west] {failed} (stop1);    
    \draw[->] (c1) -- node[anchor=south] {if Assessor Assessment} (ass1);
    \draw[->] (ass1) to (ass2);        
    \draw[->] (ass2) to (stop2);    
    % \draw[->] (c1.south) to[out=180,in=180] (c0.south);
\end{tikzpicture}
